...
* Only **Windows** is supported. Sorry, no Linux, Mac OS, Android, or anything else. Mostly due to BlackBone, but some parts of RemoteMono itself would also need a few changes.
* Currently, BlackBone only builds in **Visual Studio 2017+**, so that's what you need for RemoteMono as well. RemoteMono uses C++17 features heavily, so older versions of Visual Studio are not supported. I would love to add support for MinGW or Clang, but as long as RemoteMono depends on BlackBone and BlackBone doesn't support it, it's not gonna happen.

If you want to build the unit tests, sample project or documentation, there are additional requirements:

* For the unit tests and/or the sample project, you will need [CMake](https://cmake.org/). You *don't* need it for using RemoteMono itself.
* For the unit tests, you will need [Google Test](https://github.com/google/googletest) and a working installation of [Mono](https://www.mono-project.com/).
* For building the documentation, you will need [Doxygen](https://www.doxygen.nl/index.html).
...
While you don't need to build RemoteMono itself, there are a few things to keep in mind when building your own project that uses the RemoteMono headers:
...
* Do not try to include any headers in `remotemono/impl` directly unless you're ready to face hell. You should only have to include the headers in `remotemono`. You *can* however include the ones in `remotemono/impl/mono` and `remotemono/impl/visit_struct`.

When you build your own project or RemoteMono's sample project or unit tests, I suggest you compile BlackBone (and other dependencies if necessary) from source yourself, to rule out incompatibilities between compiler versions and configurations. That being said, you can find a set of prebuilt dependencies (BlackBone, Google Test) in the GitHub releases page. Try it at your own risk.

How do you use it?
------------------

RemoteMono is a header-only library, so there is no need to compile it. Just make sure that the directory containing the `remotemono` directory (which in turn contains `RMonoAPI.h`) is in your include path. That being said, RemoteMono depends on BlackBone, and you *do* need to compile that (or use some prebuilt library). You do not need the BlackBone driver, only the BlackBone library itself. At least I think you don't need it. I have never used the driver myself and everything seems to work just fine.

The main class you will be using is `RMonoAPI`, so take a look at `remotemono/impl/RMonoAPI_Def.h`. Most of the methods it provides have a 1:1 correspondence to one of the Mono Embedded API functions in the remote process, so take a look at the Mono Embedded API docs and just call the corresponding method in `RMonoAPI`. Take a look at the comments in some of the other classes like `RMonoVariant` and `RMonoVariantArray` for more details. And if you want to venture into the internals (I'm not sure that you want to), you can take a look at `RMonoAPIBackend` and `RMonoAPIFunction`.

That being said, the best place to start is probably the...
...
You can find the API documentation [here](https://alemariusnexus.com/remotemono/doc/), although that may not always be the latest version. Note that it is of limited use for many of RemoteMono's internals, since Doxygen isn't capable of parsing some of the template stuff correctly (I'm probably just too stupid and lazy to configure it correctly). You might want to look at the sources directly instead.

Features
--------

* It's a **header-only** library, so you don't have to compile it before using it and you can give it a try rather quickly. I realize that this would be a better selling point if it didn't depend on BlackBone (which *does* need to be built). Meh, it counts.
...
* **Transparent handling of different ABIs** in the remote process. This means that you can use the same local application for both 32-bit and 64-bit remotes. You don't even have to build two separate versions of it.
...
* **RemoteMono is an experiment**. Before I started to write RemoteMono, I have actually never used Mono before (expect for that one time I had to write a Unity game in university), and haven't had much experience with memory hacking. So it's very likely that I've missed something very essential somewhere, and there will probably be some crashes once in a while. It's currently working for the project that I wrote it for originally, though.
* RemoteMono **only works with remote processes that export the Mono Embedded API**. Most regular Mono applications *do not* export it. The most common example of remotes that *do* export it are most (all?) Unity games.
* Presently, it does **not support IL2CPP**. IL2CPP is basically a Unity-specific reimplementation of certain parts of Mono, and Unity games compiled in this mode will not work with RemoteMono. The Mono API functions exposed by IL2CPP are slightly different from the original Mono API, and support for them has not been implemented yet in RemoteMono. It is *probably* not that difficult (if a little tedious) to support it, and it's something I may consider doing if and when I feel motivated enough.
* RemoteMono **does not expose every single Mono API function**. For RemoteMono to support a Mono API function, its signature has to be defined in remotemono/impl/RMonoAPIBackend_Def.h, and a small corresponding method has to be added to remotemono/impl/RMonoAPI_*.h. As long as the API function does not have a special type of parameter or return value that RemoteMono does not support yet, it's pretty easy to add it that way. If it's currently not available, that probably just means that I haven't needed it yet and was too lazy to add it. Feel free to open an issue about any functions you would like to see.
* RemoteMono can **only support API functions that are exported** in the remote process. Sometimes a remote process may use an older version of Mono that doesn't support certain functions, or functions may be missing for other reasons. In many such cases, RemoteMono can still be used, but you can't use those specific functions.
* RemoteMono is **not very resilient to programming errors**. If you pass a wrong pointer, a null value where one shouldn't be, or some other wrong value, either your local process or the remote process (or both at once) will crash very quickly, and debugging can be a bit hard sometimes. This **will** happen to you, especially when working with value types and the `RMonoVariant` class. This is memory hacking, so I hope you're used to it.
* Because it's a heavily template-based header-only library, your own source files may take quite a while to compile if they include a RemoteMono header. For this reason, **it's quite important to use precompiled headers** if you don't want to lose your sanity.
* When using BlackBone without changes, RemoteMono's RPC calls are **very slow**, meaning <100 per second. This is [an issue with BlackBone](https://github.com/DarthTon/Blackbone/issues/430), and it may be fixable by simply commenting out a single line in the BlackBone source code (see the referenced issue), which boosts this by a factor of ~100. I don't know if that is safe to do, but I did apply this change in the prebuilt dependency package and RemoteMono's unit tests and sample program seem to work just fine with it. We'll see when stuff starts breaking I guess...
